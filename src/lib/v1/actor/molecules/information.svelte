<script lang="ts">
  ...
  let { name, traits, heritage, specialization, image, squad, callSelect } = $props();
  ...
  <div class="specialization-container">
    <button class="specialization" onclick={() => callSelect('specialization')}>
      {#if specialization !== null}
        ...
        <span class="specialization-name">{foundryAdapter.localize(`specialization.${specialization}.name`)}</span>
      {:else}
        <span class="specialization-name">{foundryAdapter.localize(`specialization.select`)}</span>
      {/if}
    </button>
  </div>
  ...
  <div class="heritage-container">
    <button class="heritage" onclick={() => callSelect('heritage')}>
      {#if heritage !== null}
        ...
        <span class="traits">(+{traits.length})</span>
      {:else}
        {foundryAdapter.localize(`heritage.select`)}
      {/if}
    </button>
  </div>
  {#if [LegionnaireEnum.Soldier, LegionnaireEnum.Rookie].includes(specialization)}
    <div class="squad-container">
      <button class="squad" onclick={() => callSelect('squad')}>
        {#if squad.name !== ''}
        ...
        {:else}
          <span class="squad-name">{foundryAdapter.localize('origin_squads.select')}</span>
        {/if}
      </button>
    </div>
  {/if}
