<script lang="ts">
  ...
  import { LegionnaireEnum, SpecialistEnum, type Specialization } from "src/types/actor.type";

  let { specialization, isOpen = $bindable(false), onconfirm, oncancel } = $props();

  let selected: Specialization = $state(specialization || null);

  const options = [...Object.values(SpecialistEnum), ...Object.values(LegionnaireEnum)];
  ...
  <div class="modal-backdrop">
    <div class="modal-content">
      <div class="modal-header">
        <div class="modal-title">{foundryAdapter.localize('select.specialization')}</div>
      ...
      <div class="modal-body">
        <div class="items-list">
          {#each options as option}
            <label class="item">
              <input type="radio" hidden name='specialization' value={option} bind:group={selected}/>
              <div class="item-title">
                <span class="item-icon">
                  <img src={'systems/band-of-blades/styles/assets/icons/' + option + '.svg'} alt="specialization name"/>
                </span>
                {foundryAdapter.localize(`specialization.${option}.name`)}
              </div>
              <div class="item-description">
                <div class="item-advancement">
                  {@html foundryAdapter.localize(`specialization.${option}.action`)}
                </div>
                <div class="item-action">
                  {@html foundryAdapter.localize(`specialization.${option}.advancement`)}
                  {#if foundryAdapter.hasLocale(`specialization.${option}.startup`)}
                    {@html foundryAdapter.localize(`specialization.${option}.startup`)}
                  {/if}
                </div>
              </div>
            </label>
